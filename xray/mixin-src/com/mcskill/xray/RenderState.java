 ...
 *
 * Применение изменений из меню: выбор руд меняет только State.ores (данные),
 * мир НЕ перестраивается на каждый клик. Перестроение чанков вызывается один
 * раз: кнопкой "Сохранить" в меню руд или автоматически при закрытии меню
 * (если есть незаписанные изменения). Сам механизм пинка дальности НЕ трогаем -
 * без него чанки перестраиваются долго (единственный рабочий способ
 * мгновенного применения).
 *
